...
// Stockholm: ONE feed from SL (GTFS Regional, the detailed one with shapes)
// covering the whole Stockholm County, cut down to the built-up region by a
// precomputed allowlist (scope.mjs → data/scope.json). Four modes out of the
// one bundle: SL's buses navy, the six lokalbanor — Spårväg City,
// Nockebybanan, Lidingöbanan, Saltsjöbanan, Roslagsbanan and Tvärbanan —
// family tram red, and on the trunk side the tunnelbana and the pendeltåg in
// their own colours, drawn wide with station discs and always-on names.
// Usage: node pipeline/build.mjs [--all | lines...] [--tram all|30,T14,40]
...
// Exact-match repairs for poles a feed names badly. SL needs none: of 14 000
// stop names only nine shout, and every one of them is an acronym the city
// itself shouts (SKF the ball-bearing works, NÄL the hospital).
const NAME_FIX = {};// m — longer jumps between shape points are GTFS data gaps. Inside a real gap
...
// ONE feed, FOUR cfgs.
//
// SL publishes GTFS Regional for the whole of Stockholm County — 555 bus
// lines reaching Norrtälje and Nynäshamn, an hour and a half from the centre.
// The map is the built-up region, so pipeline/scope.mjs precomputes the
// allowlist (data/scope.json). Route types are the extended set: 700 bus,
// 900 lokalbana, 401 tunnelbana, 100 pendeltåg.
//
// Cut deliberately: the ferries (1000 — SL's own pendelbåtar and the whole
// Waxholmsbolaget archipelago fleet; the engine has no water graph), the
// thirteen Ersättningsbussar (the feed labels them in route_desc and names
// them after the rail line they stand in for — Budapest's pótló rule), and
// Närtrafiken, SL's dial-a-ride, which has numbers but no fixed run.
//
// LINE KEYS: nothing to invent. Every SL line prints its own number and the
// numbers do not repeat across modes once the replacement buses are gone.
const LBL = new Map();

// The tunnelbana's three colours are SL's own line standard; the pendeltåg
// pink is the value OSM's SL route relations carry (the feed ships no
// route_color at all). The lokalbanor keep the family's tram red even though
// SL gives each of them a colour too — colour means the MODE in this family,
// and Copenhagen's letbane set the precedent for light rail on its own
// alignment.
const SL_COLORS = {
  10: '#007DC5', 11: '#007DC5',                 // blå linjen
  13: '#D71D24', 14: '#D71D24',                 // röda linjen
  17: '#148541', 18: '#148541', 19: '#148541',  // gröna linjen
  40: '#F166A7', 41: '#F166A7', 43: '#F166A7', '43X': '#F166A7', 48: '#F166A7',
};
const isRailTrunk = (l) => l in SL_COLORS;
...
const S_BUS = new Set(SCOPE.bus), S_TRAM = new Set(SCOPE.tram),
  S_METRO = new Set(SCOPE.metro), S_PENDEL = new Set(SCOPE.pendel);

// --tram feeds three rail cfgs, told apart by the number itself: 10–19 is the
// tunnelbana, 40–48 the pendeltåg, everything else a lokalbana
const railBucket = (l) => (isRailTrunk(l) ? (Number(String(l).replace(/\D+$/, '')) >= 40 ? 'pendel' : 'metro') : 'tram');
const tramSel = tramLines.filter((l) => l !== 'all' && railBucket(l) === 'tram');
const metroSel = tramLines.filter((l) => railBucket(l) === 'metro');
const pendelSel = tramLines.filter((l) => railBucket(l) === 'pendel');

const MODES = [{
  mode: 'bus', label: 'buses', graphMode: 'road',
  // Stockholm and its kommuner — 4 × 4 tiles cut out of the Geofabrik extract
  // (see pipeline/pbf-tiles.py); merged at load, ways deduped by id
  osmFiles: Array.from({ length: 16 }, (_, i) => `data/osm/tiles/t${i + 1}.json`),
  color: '#0059a9', colorDark: '#00294f',
  all: busAll, lines: busList.length ? busList : (busAll ? [] : ['4']),
  feeds: [
    { tag: 'sl', dir: 'data/gtfs', routeTypes: ['700'],
      skipRoute: (r) => !S_BUS.has(r.route_id)
        || r.route_desc === 'Ersättningsbuss' || r.route_desc === 'Närtrafiken',
      mapKey: (sn) => sn || null },
  ],
}];
const tramAll = tramLines.length === 1 && tramLines[0] === 'all';
if (tramAll || tramSel.length) MODES.push({
  // the six lokalbanor: Spårväg City (7) and Nockebybanan (12) run in the
  // street, Lidingöbanan (21), Saltsjöbanan (25/26), Roslagsbanan (27/28/29)
  // and Tvärbanan (30/31) on their own alignments — OSM tags the lot
  // tram or light_rail, and the 891 mm Roslagsbanan needs the narrow-gauge
  // rail too
  mode: 'tram', label: 'lokalbanor', osmFile: 'data/osm/stockholm-rail.json',
  graphMode: 'tram', railKeep: new Set(['tram', 'light_rail']),
  // Two of the six do not ride tram track at all. Roslagsbanan is 891 mm —
  // railway=narrow_gauge — and Saltsjöbanan is standard-gauge heavy rail,
  // tagged railway=rail like the mainline beside it. The discriminator is
  // OSM's own usage tag: Saltsjöbanan is usage=branch, Västra stambanan and
  // Citybanan are usage=main, so the mainline stays out of this graph and
  // Tvärbanan cannot snap onto the pendeltåg tracks it runs beside.
  railExtra: (e) => {
    const k = railKind(e.tags);
    return k === 'narrow_gauge' || (k === 'rail' && e.tags?.usage !== 'main');
  },
  color: '#d6212b', colorDark: '#7c1116',
  all: tramAll, lines: tramAll ? [] : tramSel,
  feeds: [
    { tag: 'sl', dir: 'data/gtfs', routeTypes: ['900'],
      skipRoute: (r) => !S_TRAM.has(r.route_id), mapKey: (sn) => sn || null },
  ],
});
if (tramAll || metroSel.length) MODES.push({
  mode: 'tram', label: 'tunnelbana', osmFile: 'data/osm/stockholm-rail.json',
  graphMode: 'tram', railKeep: new Set(['subway']),
  color: '#d6212b', colorDark: '#7c1116',
  all: tramAll, lines: tramAll ? [] : metroSel,
  feeds: [
    // no allVariants: every tunnelbana line is one corridor end to end — where
    // a branch exists it carries its own number (10 vs 11, 13 vs 14, 17/18/19)
    // — so the 81 stop-patterns the feed ships are short-turns, and drawing
    // them all put 27 000 km on a network of about one thousand
    { tag: 'sl', dir: 'data/gtfs', routeTypes: ['401'],
      skipRoute: (r) => !S_METRO.has(r.route_id), mapKey: (sn) => sn || null,
      lineColor: (k) => SL_COLORS[k] },
  ],
});
if (tramAll || pendelSel.length) MODES.push({
  // the pendeltåg rides mainline rail; allMetro gives every line the trunk
  // treatment without an M-prefix to key off, exactly as Copenhagen's S-tog
  mode: 'tram', label: 'pendeltåg', osmFile: 'data/osm/stockholm-rail.json',
  graphMode: 'tram', railKeep: new Set(['rail']),
  allMetro: true,
  color: '#d6212b', colorDark: '#7c1116',
  all: tramAll, lines: tramAll ? [] : pendelSel,
  feeds: [
    { tag: 'sl', dir: 'data/gtfs', routeTypes: ['100'],
      skipRoute: (r) => !S_PENDEL.has(r.route_id), mapKey: (sn) => sn || null,
      lineColor: (k) => SL_COLORS[k] },
  ],
});
...
async function processMode(cfg) {
  ...
  for (const feed of cfg.feeds) {
    ...
    // Samtrafiken's Swedish feeds ship no trip_headsign AT ALL, so every
    // direction would go nameless — and the panel's direction list, the chip
    // tooltips and the journey planner all print it. Where the feed is silent
    // the last stop of the drawn pattern IS the headsign: that is what the
    // destination blind says.
    for (const r of feedReps) {
...
await (await import('./night.mjs')).nightPass(outDir, /^9\d$/, { sort: false });
